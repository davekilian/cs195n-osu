...
public class RepeatDemo extends DemoSlide
{
	private TexturedQuad cap, fill, nub, repeat;
	...
	public void draw(Kernel kernel, float time, float slidetime) 
	{
		...
		if (cap == null)
		{
			...
			repeat = new TexturedQuad(BitmapFactory.decodeResource(kernel.getActivity().getResources(), R.drawable.slider_return, opt));
		...
		{
			float pos = 3.f * (slidetime - 7.5f) / 2.5f;
			int repeat = (int)pos;
			pos -= repeat;
			if ((repeat & 1) != 0)
				pos = 1.f - pos;
			
			startarrow = repeat < 2 ? 1.f : 0.f;
			endarrow = repeat < 1 ? 1.f : 0.f;
		...
		repeat.getTranslation().x = sliderPoints[0];
		repeat.getTranslation().y = sliderPoints[1];
		repeat.setRotation(0.f);
		repeat.setAlpha(startarrow);
		repeat.draw(kernel);
		
		repeat.getTranslation().x = sliderPoints[2];
		repeat.getTranslation().y = sliderPoints[3];
		repeat.setRotation(180.f);
		repeat.setAlpha(endarrow);
		repeat.draw(kernel);
